...
Use a Docker image with an editor and connect the volume container:
    ...
    docker run -v $OVPN_DATA:/etc/openvpn --rm -it truffelnl/docker-openvpn-ng vi /etc/openvpn/openvpn.conf
